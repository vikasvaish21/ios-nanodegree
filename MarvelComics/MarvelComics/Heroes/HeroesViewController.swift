...
class HeroesViewController: UIViewController {
    ...
    func loadHeroes(reset: Bool = false, withIndicator: Bool = false) {
        ...
        MarvelAPIManager.sharedInstance.getCharacters(nameStartWith: heroNameSearch, page: page) { (result) in
            switch result {
            case .failure(let errorMessage):
                self.currentPage = self.currentPage > 0 ? self.currentPage - 1 : 0
                self.displayError(errorMessage)
            case .success(let heroDetailContainer):
                guard let heroes = heroDetailContainer.results, let total = heroDetailContainer.total else {
    ...
    func searchHero(with name: String) {
        ...
        MarvelAPIManager.sharedInstance.getCharacters(nameStartWith: name) { (result) in
            switch result {
            case .failure(let errorMessage):
                self.displayError(errorMessage)
    ...
    func save(hero: Hero, heroImage: UIImageView?, completion: @escaping (_ myHero: MyHero?, _ error: Error?) -> Void) {
        CoreDataStack.sharedInstance.performViewTask { viewContext in
            ...
            myHero.comicCount = Int32(hero.comics?.available ?? 0)
    ...
    func navToHeroDetails(with hero: Hero, heroImage: UIImage?) {
        let controller = HeroDetailViewController.instanceFromStoryboard()
